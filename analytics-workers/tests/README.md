...
`test_config.py` protects Compose-style numeric environment parsing and bounds.
...
and unique-key integration coverage. This tier also bootstraps `init.sql`,
applies the migration twice, validates the resulting catalog, and exercises the
content-free source health check. Tests never contact OpenAI.
