...
| File | Coverage |
| --- | --- |
| <code>test_ingestion_api.py</code> | Extraction-only opt-out, deterministic page indexing, and sanitized 422/503 boundaries. |
| <code>test_ocr.py</code> | EasyOCR disablement and the Tesseract-only path. |
| <code>test_retrieval.py</code> | Tokenization, stable IDs, hash normalization, lexical reranking, sparse upsert/rehydration, fusion ordering, and provenance. |
| <code>test_sql_sandbox.py</code> | Statement/function blocking, limits, schema introspection, execution, and rollback. |
| <code>test_orchestrator.py</code> | Planning, specialist order, memory, skipped/gapped workflows, synthesis, and arbitration integration. |
| <code>test_consensus.py</code> | Quorum, retries, timeout, adjudication, remediation, deterministic pass/block policy, and fail-closed verdicts. |
...
| <code>test_runtime_execution_api.py</code> | Restricted execution context, dependency/thread delegation, trace pagination/detail validation, missing artifacts, and response redaction. |
